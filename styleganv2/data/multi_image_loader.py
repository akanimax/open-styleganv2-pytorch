""" Module for the multi-image data loading pipeline for the model to train """
...
def get_transform(
    new_size=(256, 256), flip_horizontal=False
...
def _get_files(path: Path, rec: bool = False) -> List[Path]:
    """
    private helper function to search the given directory and obtain all the files
    in it's structure.
    Args:
    ...
    for possible_file in path.iterdir():
        if possible_file.is_file():
            ...
            files.extend(_get_files(possible_file))
    return files
...
class ImageDirectoryDataset(Dataset):
    ...
    Args:
        ...
        input_data_range: range in which input data lies
        output_data_range: range in which the data values lie for training
    ...
    def __init__(
        ...
        self.files = _get_files(data_dir, rec_dir)
